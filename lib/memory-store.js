...
async function search(
  ...
  if (mode === "fts") {
    ...
    // Always run through rrfMerge — even for short queries (no semantic
    // pass) and even when semantic returned nothing. Hybrid-mode rows must
    // carry an `rrf` score for downstream consumers; without this, FTS-only
    // results have `.rrf === undefined` and the
    // memory-store.test.mjs "hybrid search merges FTS + semantic via RRF"
    // assertion fires.
    results = rrfMerge(fts, sem, { limit: poolLimit });
  }
