...
/// **Merlin's** Supabase (not Spotter's own project). Verbatim port of
/// `client/Merlin/HealthKitManager.swift` from the merlin repo — only
/// the supabase client reference is swapped (`SpotterSupabase` here vs
/// `SupabaseManager` there). Rationale: Spotter is foregrounded for far
/// longer stretches during workouts, so running this same sync from
/// Spotter dramatically expands coverage compared to the Merlin app
/// alone.
///
/// Three paths:
///
...
final class HealthKitManager: @unchecked Sendable {
    ...
    /// Vendor id captured on the main actor at `start()` so the rest of the
    /// pipeline can build `device_id` strings without bouncing back to
    /// MainActor on every upload (Spotter compiles with strict concurrency
    /// complete; Merlin doesn't, which is why this isn't an issue there).
    private nonisolated(unsafe) var cachedVendorId: String?
    ...
    private var rawSampleTypes: [HKSampleType] {
        ...
        // populating this. Apple ships this as a quantity (count), not a category.
    ...
    func start() {
        ...
        // Cache the vendor id on the main actor (UIDevice.current is MainActor-
        // isolated) so background sync paths can read it synchronously.
    ...
    /// Called from scene-phase .active in SpotterApp.
    ...
    /// the number of anchor keys cleared.
    @discardableResult
    ...
    private func installObservers() {
        for type in rawSampleTypes {
            ...
            let obs = HKObserverQuery(sampleType: type, predicate: nil) { [weak self] _, completion, err in
                ...
                // Bridge HKObserverQueryCompletionHandler (non-Sendable) across
                // the Task hop. The completion block is single-shot and just
                // signals HealthKit to expire the silent push — safe to ship
                // through @unchecked Sendable.
                let bridge = ObserverCompletionBridge(handler: completion)
                Task {
    ...
    private func sendBatches<T: Encodable>(_ rows: [T], table: String, conflict: String) async throws {
        guard !rows.isEmpty else { return }
        try await SpotterSupabase.ensureAuthenticated()
        for chunk in rows.chunked(into: batchSize) {
            try await SpotterSupabase.shared
                .from(table)
...
private struct ObserverCompletionBridge: @unchecked Sendable {
